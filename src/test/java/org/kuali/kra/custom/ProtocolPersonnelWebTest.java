...
public class ProtocolPersonnelWebTest extends ProtocolWebTestBase{
    ...
    public void testDeleteUnitFromPerson() throws Exception {
        ...
        // TODO : this is not working for delete because data has not persisted yet.
        // so, use selectedPerson.getProtocolUnits().remove(protocolUnit) is not good
        // because both units probably are equal because pk is null, and remove is removing the first match one.
        // 'protocolunit' is IU-UNIV, but actually remove the first one '000001'
        assertFalse(personnelPage.asText().contains(UNIT_NUMBER));
        saveAndSearchDoc(personnelPage);
    }
